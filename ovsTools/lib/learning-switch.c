...
void queue_tx(struct lswitch *, struct rconn *, struct ofpbuf *); //Junos - static removed

static void send_features_request(struct lswitch *, struct rconn *);

void send_default_flows(struct lswitch *sw, struct rconn *rconn,		//Junos changes
                               FILE *default_flows);
...
void //Junos - static removed
queue_tx(struct lswitch *sw, struct rconn *rconn, struct ofpbuf *b)
{
    int retval = rconn_send_with_limit(rconn, b, sw->queued, 10000);
...
//    VLOG_INFO_RL("XXXXXXX: Packets sent %u", rconn_packets_sent(rconn));
}
