...
// Junos changes : begin ->
int parse_packet_out_params (char *args)
{
    int output_port;
    char *tmp;

    tmp = strstr(args, "output_port");

    if (!tmp) {
        ovs_fatal(0, "you must specify an output port parameter");
    }
    *tmp = '\0';

    tmp = strchr(tmp + 1, '=');
    if (!tmp) {
        ovs_fatal(0, "must specify an output port number");
    }
    tmp++;
    output_port = atoi(tmp);
    if(!output_port) {
        ovs_fatal(0, "wrong output_port parameter format");
    }
}
// Junos changes : end <-
