...
static void
do_junos_add_flow (int argc OVS_UNUSED, char *argv[])
{
    ...
    VLOG_INFO("do_junos_add_flow: args received: %s", argv[2]);

    copy_of_args = xstrdup(argv[2]);

    /* Parse a copy of received args -> to assure 
     * that string sent to controller is correct  */
    make_openflow(sizeof *ofm, OFPT_FLOW_MOD, &b);
    parse_ofp_str(copy_of_args, &match, b,
                      NULL, NULL, &priority, &idle_timeout, &hard_timeout,
                      &cookie);
    free(copy_of_args);
    free(b);
    ...
    ret = unixctl_client_create(JUNOS_SOCKET_NAME, &client);
    if(ret) {
        VLOG_ERR("do_junos_add_flow: Unable to create unixctl client");
        return;
    }
    request = xasprintf( JUNOS_ADD_FLOW_CMD " %s", argv[2]);    
    unixctl_client_transact(client, request, &code, &reply);
    free(request);
}

static void do_junos_del_flow(int argc, char *argv[])
{
    struct unixctl_client *client;
    char * request;
    char * reply; 
    ...
    if(argv[2]) {
        ...
        VLOG_INFO("do_junos_del_flow: args received: %s", argv[2]);

        copy_of_args = xstrdup(argv[2]);

        /* Parse a copy of received args -> to assure
         * that string sent to controller is correct  */
        ofm = make_openflow(sizeof *ofm, OFPT_FLOW_MOD, &b);
        parse_ofp_str(argc > 2 ? copy_of_args : "", &ofm->match, NULL, NULL,
                  &out_port, &priority, NULL, NULL, NULL);
        free(copy_of_args);
        free(b);
        ...
        ret = unixctl_client_create(JUNOS_SOCKET_NAME, &client);
        if(ret) {
            VLOG_ERR("do_junos_del_flow: Unable to create unixctl client");
            return;
        }
        request = xasprintf( JUNOS_DEL_FLOW_CMD " %s", argv[2]);
        unixctl_client_transact(client, request, &code, &reply);
        free(request);
    } else {
        ...
        VLOG_INFO("do_junos_del_flow: sending empty junos-del-flow request -> flush");
        request = xasprintf( JUNOS_DEL_FLOW_CMD " %s", argv[2]);
        unixctl_client_transact(client, request, &code, &reply);
        free(request);
...
static void do_junos_packet_out(int argc OVS_UNUSED, char *argv[])
{
    struct unixctl_client *client;
    char * request;
    char * reply;
    ...
    if(argv[2]) {
        VLOG_INFO("do_junos_packet_out: args received: %s", argv[2]);
        
        copy_of_args = xstrdup(argv[2]);

        parse_packet_out_params(copy_of_args);

        ret = unixctl_client_create(JUNOS_SOCKET_NAME, &client);
        if(ret) {
            VLOG_ERR("do_junos_packet_out: Unable to create unixctl client");
            return;
        }
        
        request = xasprintf( JUNOS_PACKET_OUT " %s", argv[2]);
        unixctl_client_transact(client, request, &code, &reply);
        free(request);
    }
...
static const struct command all_commands[] = {
    ...
    { "probe", 1, 1, do_probe },
    { "ping", 1, 2, do_ping },
    { "junos-add-flow", 2, 2, do_junos_add_flow },
    { "junos-del-flow", 1, 2, do_junos_del_flow },
    { "junos-packet-out", 2, 2, do_junos_packet_out },
